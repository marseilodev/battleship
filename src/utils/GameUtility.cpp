...
void benvenuto() {
  ...
  printCentered(
      "T I P S: Prima di iniziare, leggi attentamente il regolamento\n");
}
...
int selezioneDifficolta() {
  ...
  do {
    ...
    std::cout << "\nSelezionare una delle seguenti opzioni (La selezione della "
              ...
              << std::endl;
    std::cout
        << "(1) Principiante: Deciderai tu dove le navi sono posizionate. "
           "Troppo facile, vero?"
        << std::endl;
    std::cout
        << "(2) Intermedio: Le navi saranno disposte in posizioni random lungo "
    ...
    bool isNumber =
        !input.empty() && std::all_of(input.begin(), input.end(), ::isdigit);
    ...
    if (isNumber) {
      ...
      difficoltaValida =
          false; // Qualora la difficoltà inserita non è valida, avvisa
...
int inserimentoDimensioni() {
  ...
  do {
    ...
    if (!dimensioneValida) {
      printCentered("*** E R R O R E ***");
      printCentered(
          "Opzione non valida, selezionare dimensione del campo "
...
void Partita() {
  ...
  do {
    ...
    CampoDiBattaglia *campo =
        nullptr; // Definizione puntatore che non punta a niente per evita che
    ...
    if (sceltaDifficolta == 4) {
      campo = new CampoIntermedio(
          dimensione);          // Crea dimensione del campo (PRIMA: 0)
      campo->caricamentoFile(); // Puntatore che carica i salvataggi sul file
      dimensione =
          campo->getDimensioneGriglia(); // Riaggiorna dimensione del file
    ...
    if (!campo) {
      printCentered("*** E R R O R E ***");
      printCentered(
          "Errore durante la creazione del campo di battaglia! Riprovare...");
      return;
    }
    ...
    while (!campo->tutteNaviAffondate()) {
      ...
      if (sceltaDifficolta == 3 && campo->getSparoNonSalvato()) {
        CampoAvanzato *campoAvanzato = dynamic_cast<CampoAvanzato *>(
            campo); // Trasformazione puntatore CampoDiBattaglia a CampoAvanzato
                    // dinamicamente

        // Verifica se campo è diventato oggetto di CampoAvanzato
        if (campoAvanzato) {
          campoAvanzato
              ->muoviNavi(); // Puntatore richiama metodo di spostamento navi
      ...
      if (campo->tutteNaviAffondate()) {
        ...
        continuaGioco =
            gestioneChiusura(campo); // Richiama funzione necessaria a
...
void PartitaMultiplayer() {
  bool rigioca =
      false; // Variabile booleana per controllare se l'utente vuole rigiocare
  ...
  do {
    ...
    campoUtente->stampaGriglia(
        false); // Puntatore Utente che richiama metodo stampa la griglia (senza
    ...
    bool turnoUtente =
        true; // Variabile booleana per indicare il turno dell'utente
    ...
    do {
      if (turnoUtente) {
        ...
        do {
          ...
          bool isNumberX = !inputX.empty() &&
                           std::all_of(inputX.begin(), inputX.end(), ::isdigit);
          bool isNumberY = !inputY.empty() &&
                           std::all_of(inputY.begin(), inputY.end(), ::isdigit);
        ...
        campoUtente->verificaSparoComputer(
            xAI, yAI); // Puntatore richiama metodo per verificare che il colpo
                       // sia andato a segno

        std::cout << "\nIl computer ha sparato nelle coordinate: ( " << xAI
                  << ", " << yAI << " )" << std::endl;
        ...
        if (campoUtente->tutteNaviAffondate()) {
          ...
          printCentered(
              "L'AI ha vinto!, Peccato, potrai rifarti la prossima volta...");
          gameOver = true; // Termina il gioco completamente
          rigioca = false;
        } else {
...
int getTerminal() {
  int lunghezza, larghezza; // Definisco la dimensione del terminale

  // Definisco comando per leggere le dimensioni del terminale
  FILE *terminale = popen("stty size", "r");
  if (!terminale) {
    return 80; // valore base del terminale
  }
  // Scannerizza la grandezza del terminale in maniera dinamica
  fscanf(terminale, "%d %d", &lunghezza, &larghezza);
  pclose(terminale);
  return larghezza;
}
